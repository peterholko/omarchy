...
A child install filters the web from its first boot: every lookup is answered by [Cloudflare for Families](https://developers.cloudflare.com/1.1.1.1/setup/#1111-for-families), which drops malware and adult sites without a list to keep, and the deny list starts empty. `sudo omarchy-parent dns` is the parent's command for it. `sudo omarchy-parent dns deny tiktok.com` refuses a site of your own choosing under that default, denylist mode. `sudo omarchy-parent dns allowlist` refuses everything except the domains you allow, `sudo omarchy-parent dns allow khanacademy.org`, plus a short list the machine needs for updates and time. A site is many domains: YouTube needs `youtube.com`, `googlevideo.com`, `ytimg.com`, `ggpht.com`, and `google.com`, so `sudo omarchy-parent dns log` shows the names a page just asked for and was refused, most often first, ready to allow. A denied name wins over an allowed one, so `allow google.com` with `deny mail.google.com` does what it says. `sudo omarchy-parent dns upstream auto` answers allowed names through the network's own servers instead of Cloudflare for Families, which a sign-in page that blocks outside DNS can need until the parent is signed in; `upstream family` puts Cloudflare back. `sudo omarchy-parent dns off` removes all of it and keeps your lists.

The lists are `/etc/omarchy/parent/dns.allow` and `dns.deny`, one domain per line; edit them and run `sudo omarchy-parent dns apply`. An entry with a path is a page, not a domain, and the browser refuses it rather than the resolver, which cannot see paths: `sudo omarchy-parent dns deny youtube.com/shorts` keeps YouTube open but closes the Shorts feed, in Chromium and Firefox alike, under either mode. A page the kid reaches another way, the same short opened as an ordinary video for instance, is not caught; the domain lists are the firm ground, the page list the trim. The filter runs as root: the kid's session cannot reach another resolver on the usual ports, the browsers are told not to use DNS over HTTPS, and a stopped filter means no web rather than an open one. It does not stop a VPN or proxy app (installs ask for the parent password), a browser extension that tunnels, or an address typed as numbers; the app allowlist is the other half. After installing another browser, run `sudo omarchy-parent dns apply` so its policy is written too.

The filter also keeps a log of every site the laptop asked for, in the system journal, where the kid can neither read nor clear it. `sudo omarchy-parent dns history` lists the last day's sites, most often first, with when each was last seen; `history 7` covers a week. It records sites, not pages, and only while the filter is on. Tell your kid the log is there: the point of a preteen's setup is safety, not surveillance, and a log they know about is a boundary rather than a trap.
