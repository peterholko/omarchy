...
A child install can filter the web by domain with `sudo omarchy-parent dns`. `sudo omarchy-parent dns denylist` lets everything through except the domains you deny, `sudo omarchy-parent dns deny tiktok.com` for instance. `sudo omarchy-parent dns allowlist` refuses everything except the domains you allow, `sudo omarchy-parent dns allow khanacademy.org`, plus a short list the machine needs for updates and time. A site is many domains: YouTube needs `youtube.com`, `googlevideo.com`, `ytimg.com`, `ggpht.com`, and `google.com`, so `sudo omarchy-parent dns log` shows the names a page just asked for and was refused, most often first, ready to allow. A denied name wins over an allowed one, so `allow google.com` with `deny mail.google.com` does what it says. `sudo omarchy-parent dns upstream family` answers allowed names through Cloudflare for Families, which also drops malware and adult sites without a list to keep. `sudo omarchy-parent dns off` removes all of it and keeps your lists.
