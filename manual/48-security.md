...
A child install also comes set up for a kid rather than for work. The keyboard shortcuts for the chat, mail, and social web apps (ChatGPT, Grok, HEY, WhatsApp, Google Messages, X) and for Signal, Docker, and 1Password are not there; YouTube, Google Photos, Google Maps, music, Obsidian, and the writing app are, and Khan Academy (`Super + Shift + K`) and Wikipedia (`Super + Shift + Alt + K`) join them. The menu's Community link to Discord is off. The theme switcher offers a child-friendly set, starting on Bubblegum, and the kid can still add themes of their own; the web filter and the app list, below, are the parent's tools for the rest.
