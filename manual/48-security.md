...
A child install can make the kid earn her screen time with arithmetic. Turn it on with `sudo omarchy-parent time on`. From then on her time counts down while the screen is unlocked, and when it runs out the screen locks. She unlocks it with her own password, and instead of her desktop she gets _Math time_: a full-screen session of five questions that earns thirty minutes when every answer is right, six minutes per correct answer, up to 120 minutes a day. A wrong answer gets a second try, then the answer is shown and the next question comes. Omacalc is closed and cannot be opened while _Math time_ is on screen, but remains available normally. When the session is done she sees how she did, how long it took, and what she has banked, and her desktop is back. She can also open _Math time_ from the Omarchy menu whenever she likes, to bank time before it runs out; the lock screen always shows what she has left.

The questions start at grade 5 arithmetic: adding and subtracting large numbers, multiplying two-digit numbers, and exact long division. `sudo omarchy-parent time level grade6` adds bigger multiplication, two-digit divisors, and order of operations. Tune the session with `sudo omarchy-parent time questions 4` and `rate 5` (minutes per correct answer), and the day with `cap 90` (minutes earnable per day) and `free 30` (minutes handed out each morning). `sudo omarchy-parent time grant 30` banks time outright, for a homework night or a reward; `status` and `log` show where she stands, and `sudo omarchy-parent time report` prints the day's use, earnings, and every question with its answer and how long it took, kept for each finished day under root's state. `sudo omarchy-parent time off` stops the countdown and keeps the history.
...
The countdown, the locking, and the checking of answers all run as root, and while she has no time left a root task checks every few seconds that _Math time_ is what is on her screen and locks the session again if it is not, so closing the session or killing the shell buys nothing. The screen stays on while _Math time_ is open, paper and pencil included. Sleeping and a locked screen do not count. A kid who writes a program to solve her own arithmetic has, in fairness, earned the time.
