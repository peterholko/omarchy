...
assert(!/property\s+\w+\s+(?:left|right|top|bottom|horizontalCenter|verticalCenter|baseline)\s*:/.test(plugin), 'the Item root does not redeclare a final anchor-line property')
assert(/property int correctAnswers: 0/.test(plugin), 'the session keeps its correct-answer count under a safe property name')
