...
assert(/logEvent\("unlocked"\)[\s\S]*?if \(childInstall && timeGate\.enabled && timeGate\.gated\) \{[\s\S]*?summonMath\(\)/.test(service), 'an unlock with no time left hands the session to the plugin')
