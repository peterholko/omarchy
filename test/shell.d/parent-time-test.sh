...
eval "$(sed -n '/^OMARCHY_PATH=/,/^USER_NAME=""/p' "$parent_time")"
...
: >"$CALLS"
time_school mon-fri 08:00-15:30 sat 9:00-11:00 >/dev/null
grep -q 'blocked_periods' "$CALLS" || fail "school patches the periods"
periods=$(sed -n 's/^client --user kid config patch //p' "$CALLS" | tail -1)
[[ $(jq -r '.blocked_periods | length' <<<"$periods") == 3 ]] || fail "school keeps the bedtime and adds both windows" "$periods"
[[ $(jq -c '.blocked_periods[1] | [.label, .mode, .days, .start, .end]' <<<"$periods") == '["School","free",["mon","tue","wed","thu","fri"],"08:00","15:30"]' ]] || fail "a school window is free time on its days" "$periods"
[[ $(jq -c '.blocked_periods[2].days' <<<"$periods") == '["sat"]' ]] || fail "the second window keeps its own day" "$periods"
...
: >"$CALLS"
time_school off >/dev/null
[[ $(sed -n 's/^client --user kid config patch //p' "$CALLS" | tail -1 | jq -r '.blocked_periods | map(select(.label == "School")) | length') == 0 ]] || fail "school off drops the school windows and keeps the rest"
pass "school hours and bedtime become the daemon's periods"
