# Kids mode, DNS filter: `sudo omarchy-parent dns`

Rev 1, 2026-09-01. Branch `kids/dns`, cut from `kids/child-profile`; lands after phase 0 as its own PR. DHH's list for `omarchy-parent` starts with "DNS whitelist"; Peter asked for both an allowlist and a denylist, with the app allowlist (`plans/kids-apps.md`) next and screen time (`plans/kids-screen-time.md`) last.
...
```
sudo omarchy-parent dns                        # status
sudo omarchy-parent dns denylist               # on: everything resolves except the deny list
sudo omarchy-parent dns allowlist              # on: only the allow list and the system list resolve
sudo omarchy-parent dns off
sudo omarchy-parent dns allow DOMAIN...        # add to the allow list (and drop from deny)
sudo omarchy-parent dns deny DOMAIN...         # add to the deny list (and drop from allow)
sudo omarchy-parent dns deny youtube.com/shorts  # an entry with a path: refused by the browser, not the resolver
sudo omarchy-parent dns remove DOMAIN...
sudo omarchy-parent dns list
sudo omarchy-parent dns log [N]                # the names that were refused, most often first
sudo omarchy-parent dns history [DAYS] [N]     # every site asked for, most often first, with when it was last seen
sudo omarchy-parent dns upstream auto|family   # who answers for allowed names
sudo omarchy-parent dns apply                  # after editing the lists by hand
```

## Decisions

- **Off by default.** A feature switches on with its own command, like every `omarchy-parent` feature. The manual and the first-boot summary tell the parent it exists. A denylist with nothing in it is the same as off, so there is no "family preset" shipped; the `family` upstream (below) is the one-line version of that.
...
- **`upstream family`** points dnsmasq at Cloudflare for Families (1.1.1.3, 1.0.0.3) instead of the machine's servers, which adds Cloudflare's malware and adult-content blocking under either mode without a list to maintain. `auto` is the default.
...
| What | Name |
| --- | --- |
| Command | `bin/omarchy-parent-dns`, reached as `sudo omarchy-parent dns ...` |
...
| Unit | `default/parent/omarchy-parent-dns.service` → `/etc/systemd/system/` |
| Upstreams | `/run/omarchy-parent/dns/resolv.conf`, written by `omarchy-parent-dns upstreams` from the NetworkManager dispatcher `/etc/NetworkManager/dispatcher.d/50-omarchy-parent-dns` and by the unit at start |
| NetworkManager drop-in | `/etc/NetworkManager/conf.d/50-omarchy-parent-dns.conf` |
| resolved drop-in | `/etc/systemd/resolved.conf.d/50-omarchy-parent-dns.conf` |
| Firewall | `# BEGIN OMARCHY PARENT DNS` ... `# END OMARCHY PARENT DNS` in `/etc/ufw/after.rules` and `after6.rules` |
| Browser policy | `omarchy-parent-dns.json` in each present Chromium-family managed policy directory; `DNSOverHTTPS` merged into each present Firefox `policies.json` |
| Test override | `OMARCHY_PARENT_SYSROOT` prefixes every system path above (and `OMARCHY_PARENT_CONF` the settings file) |

## Design

### The command

`bin/omarchy-parent-dns`: `# omarchy:summary=Filter the web by domain, allowlist or denylist`, `requires-sudo`, self-elevating like `omarchy-parent-time`, sourcing `install/helpers/parent.sh` for the settings helpers. Child installs only; needs a booted system (`systemd_running`) and the `dnsmasq` binary and user, and says how to get them if missing.
...
bind-interfaces
user=dnsmasq
resolv-file=/run/omarchy-parent/dns/resolv.conf     # or, under upstream family: no-resolv + server=1.1.1.3 ...
cache-size=2000
domain-needed
bogus-priv
stop-dns-rebind
...
# /etc/NetworkManager/conf.d/50-omarchy-parent-dns.conf
[main]
dns=none
